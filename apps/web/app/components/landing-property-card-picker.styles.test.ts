...
describe('property explorer visual contract', () => {
  ...
  it('uses compact edge-safe declaration cards through the 430px mobile range', async () => {
    ...
    expect(stylesheet).toMatch(/@media\s*\(max-width:\s*430px\)[\s\S]*?\.landing-action-sheet \.landing-property-search input\s*\{[^}]*min-height:\s*44px[^}]*font-size:\s*16px/s);
    expect(stylesheet).toMatch(/@media\s*\(max-width:\s*430px\)[\s\S]*?\.landing-action-sheet \.landing-owner-filter\s*\{[^}]*min-height:\s*44px/s);
  });
  ...
  it('keeps all mobile browse property surfaces inset and compact', async () => {
    ...
    expect(stylesheet).toMatch(/@media\s*\(max-width:\s*430px\)[\s\S]*?\.browse-property-picker \.landing-property-search input\s*\{[^}]*min-height:\s*44px[^}]*font-size:\s*16px/s);
    expect(stylesheet).toMatch(/@media\s*\(max-width:\s*430px\)[\s\S]*?\.browse-property-picker \.landing-owner-filter\s*\{[^}]*min-height:\s*44px/s);
