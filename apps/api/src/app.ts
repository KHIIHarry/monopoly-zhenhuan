...
app.post('/api/rooms/:id/requests', async (request) => { const auth = await authenticate(request); const { id } = z.object({ id: z.string() }).parse(request.params); const body = z.object({ playerId: z.string(), type: z.enum(['BUY_PROPERTY','BUILD_PROPERTY','SELL_BUILDING','MORTGAGE_PROPERTY','REDEEM_PROPERTY','SELL_PROPERTY_TO_BANK','TRADE_PROPERTY','START_REWARD','COLD_PALACE_EVENT','COMPANION_EVENT']), propertyName: z.string().optional(), targetPlayerId: z.string().optional(), amount: z.number().int().nonnegative().optional(), count: z.number().int().positive().optional(), landingId: z.string().optional() }).parse(request.body); await player(auth, id, body.playerId); const { playerId, ...action } = body; const result = await games.createRequest(gameActor(auth), id, playerId, action, idempotencyKey(request.headers['idempotency-key'])); notifyVersion(id, result); return result; });
app.post('/api/rooms/:id/transfers', async (request) => { const auth = await authenticate(request); const { id } = z.object({ id: z.string() }).parse(request.params); const body = z.object({ fromPlayerId: z.string(), toPlayerId: z.string(), amount: z.number().int().positive() }).parse(request.body); await player(auth, id, body.fromPlayerId); const result = await games.transfer(gameActor(auth), id, body.fromPlayerId, body.toPlayerId, body.amount, idempotencyKey(request.headers['idempotency-key'])); notifyVersion(id, result); return result; });
...
app.post('/api/rooms/:id/bank/consume-skip-turn', async (request) => { const auth = await authenticate(request); const { id } = z.object({ id: z.string() }).parse(request.params); await bank(auth, id); const body = z.object({ playerId: z.string(), reason: z.string().trim().min(1) }).parse(request.body); const result = await games.consumeSkip(gameActor(auth), id, body.playerId, idempotencyKey(request.headers['idempotency-key']), body.reason); notifyVersion(id, result); return result; });
